...
def _pytypes_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "PYTYPES_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    pytypesinfo = MylangInfo(
...
pytypes_toolchain = rule(
    implementation = _pytypes_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a pytypes compiler/runtime toolchain.
